...
export default function HotelPage() {
  ...
  useEffect(() => {
    axios.get(`https://travel-scout-640e7-default-rtdb.asia-southeast1.firebasedatabase.app/${id}`)
      ...
      {hotel.reviews.length > 0 ? (
        <ul className="mt-2">
          {hotel.reviews.map((review, index) => (
            <li key={index} className="border-b py-2 text-gray-700">{review.text}</li>
          ))}
        </ul>
      ) : (
